...
def dump_jsonl(data: Sequence[dict[str, Any]], fd: IO[str]) -> None:
    """Dump the sequence of dicts linewise to `fd`."""
    for line in data:
        fd.write(json.dumps(line) + "\n")


def get_package_name(recipe: StrPath) -> str:
...
def remove_empty_folder(folder: StrPath) -> None:
...
def get_local_ts(iso_dt: str | datetime) -> int:
    ...
    if updated_tz is None:  # pragma: no cover
        # Defensive: astimezone(local_tz) always sets tzinfo to local_tz, which is
        # itself derived from datetime.now().astimezone(); this can't be None on
        # any system able to determine its own local timezone.
        raise ValueError("local timezone transfer failed")
    return int(updated_tz.fromutc(updated_dt).timestamp())


def clean_output(  # noqa: C901, PLR0912, PLR0913, PLR0917
    recipe: StrPath,
    output_dir: StrPath,
    ...
    package = get_package_name(recipe)
    timestamp = get_local_ts(variant_line["timestamp"])
    variant = get_msg(variant_line).removeprefix(BLD_PREFIX)
    ...
    if not_skipped:
        src_msg = next(msg for line in logs[2:] if (msg := get_msg(line)).startswith(SRC_PREFIX))
    ...
    if clean_src_cache:
        remove_empty_folder(output_dir / "src_cache")

    if clean_bld_cache:
        if bld_dir_exists:
            shutil.rmtree(bld_dir)
            logger.debug("Removed build cache: %s", bld_dir)
        remove_empty_folder(output_dir / "bld")
    elif bld_dir_exists:
...
def rattler_build(  # noqa: C901, PLR0913, PLR0917
    recipe: StrPath = Path(),
    ...
    no_test: bool = False,  # TODO(tim25651): deprecated warning
...
def optimized_rattler_build(  # noqa: PLR0913, PLR0917
    recipe: StrPath,
    output_dir: StrPath,
    clean_bld_cache: bool = True,
    clean_src_cache: bool = True,
    ...
    """
    test_map: dict[bool | Literal["native"], Literal["skip", "native", "native-and-emulated"]] = {
        False: "skip",
        "native": "native",
        True: "native-and-emulated",
    }
    ...
    if code == 0:
        ...
        if run_conda_index:
            ...
            if has_conda:
                ...
                conda_index.api.update_index(output_dir)  # type: ignore[no-untyped-call]
                logger.debug("conda_index run on %s", output_dir)
            else:
                logger.warning("conda_index not available. indexing skipped.")
    else:
        logger.error("rattler-build failed. directories not removed. check returned logs.")
        if check:
